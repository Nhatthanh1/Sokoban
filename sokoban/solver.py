...
def breadthFirstSearch(gameState):
    ...
    while  frontier:
        # We get node and node_action from the first of the queue
        # because BFS use queue instead of stack in DFS
        node= frontier.pop(0)
        ...
        if node[-1] not in exploredSet:
            ...
            exploredSet.add(node[-1])
            # We loop through all next node (moves) from current node (player position)
            for action in legalActions(node[-1][0], node[-1][1]):
                # Get next node
                newPosPlayer, newPosBox = updateState(node[-1][0], node[-1][1], action)
